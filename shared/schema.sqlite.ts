...
// import { randomUUID } from "crypto"; // Moved up 
// Shims
...
// Timestamp shim handles the builder creation, but .$defaultFn(() => new Date()) replacement happens in text
...
export const rateLimitBuckets = pgTable("rate_limit_buckets", {
  ...
  lastRefill: timestamp("last_refill").$defaultFn(() => new Date()).notNull(),
  updatedAt: timestamp("updated_at").$defaultFn(() => new Date()).notNull(),
}, (table) => [
...
export const jobs = pgTable("jobs", {
  ...
  nextRunAt: timestamp("next_run_at").$defaultFn(() => new Date()).notNull(),
...
export const sourceVersions = pgTable("source_versions", {
  ...
  ingestedAt: timestamp("ingested_at").$defaultFn(() => new Date()).notNull(),
  createdAt: timestamp("created_at").$defaultFn(() => new Date()).notNull(),
}, (table) => [
