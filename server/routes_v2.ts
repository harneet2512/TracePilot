...
export async function registerRoutes(
  ...
  app.get("/api/sources/:sourceId", async (req, res) => {
    try {
      ...
      const source = await storage.getActiveSourceVersion(sourceId);

      if (!source) {
        return res.status(404).json({ error: "Source not found" });
      }

      // Get the latest version for this source
      const version = await storage.getLatestSourceVersion(sourceId);
  ...
  app.post("/api/debug/oauth/reset/google", async (req, res) => {
    try {
      // Get user ID from query param (dev mode) or session
      const userId = req.query.skip_auth === "1"
        ? (req.query.userId as string || "cf52c3a6-8cd0-4c5c-82fe-8d3a5e4d8e9a") // Default dev user
        : (req as any).user?.id;
      ...
      const result = await storage.deleteUserConnectorAccountByType(userId, "google");
  ...
  app.post("/api/debug/oauth/reset/google/account/:accountId", async (req, res) => {
    try {
      ...
      const skipAuth = req.query.skip_auth === "1";
  ...
  app.get("/api/debug/scope/:scopeId/summary", async (req, res) => {
    try {
      ...
      // Get all chunks and filter to scope sources
      const allChunks = await storage.getChunks();
      const scopeChunks = allChunks.filter((c: any) => {
        return scopeSources.some((s: any) => s.id === c.sourceId);
      });
      ...
      console.log(`[debug:summary] scopeId=${scopeId}, workspaceId=${workspaceId}, sources=${scopeSources.length}, chunks=${scopeChunks.length}`);
  ...
  app.get("/api/debug/retrieval/alignment", async (req, res) => {
    try {
      ...
      // Get sample chunks grouped by workspaceId
      const allChunks = await storage.getChunks();
      const workspaceChunks: Record<string, number> = {};
      for (const chunk of allChunks) {
        const wsId = chunk.workspaceId || "NULL";
        workspaceChunks[wsId] = (workspaceChunks[wsId] || 0) + 1;
      }

      // Get sample sources grouped by workspaceId
      const allSources = await storage.getSources();
  ...
  app.post("/api/chat", authMiddleware, chatLimiter, async (req, res) => {
    try {
      ...
      // Extract all citations from bullets into top-level array for easy frontend access
      const allCitations = result.bullets.flatMap(b => b.citations.map(c => {
        const enriched = c as any;  // Citations are enriched with url/label in agentCore
        return {
          sourceId: c.sourceId,
          chunkId: c.chunkId,
      ...
      res.json({
        ...chatResponse,
        citations: allCitations,  // Top-level citations array
...
{"answer": "...", "bullets": [], "action": null, "needsClarification": false, "clarifyingQuestions": []}`;
  ...
  app.get("/api/oauth/google", authMiddleware, (req, res) => {
    ...
    // Provider-specific base URL: GOOGLE_PUBLIC_BASE_URL > PUBLIC_BASE_URL > localhost
    ...
    // Diagnostic log for OAuth debugging
    console.log("[oauth:google] client_id=%s redirect_uri=%s", clientId, redirectUri);
    console.log("[oauth:google] env: GOOGLE_PUBLIC_BASE_URL=%s PUBLIC_BASE_URL=%s PORT=%s",
      process.env.GOOGLE_PUBLIC_BASE_URL || "unset",
      process.env.PUBLIC_BASE_URL || "unset",
  ...
  app.get("/api/oauth/google/callback", async (req, res) => {
    try {
      ...
      // Must match initiation route's provider-specific base URL
      const baseUrl = process.env.GOOGLE_PUBLIC_BASE_URL
        || process.env.PUBLIC_BASE_URL
        || `http://localhost:${process.env.PORT || 5000}`;
      const redirectUri = `${baseUrl}/api/oauth/google/callback`;
      ...
      // Log granted scopes from token response
      const grantedScopes = tokens.scope ? tokens.scope.split(" ") : [];
      console.log(`[oauth:google] Token granted scopes: ${grantedScopes.join(", ")}`);

      // Warn if drive.readonly scope is missing
      const hasDriveReadonly = grantedScopes.some(s =>
        s.includes("drive.readonly") || s.includes("drive.file") || s.includes("drive")
      );
      if (!hasDriveReadonly) {
        console.warn(`[oauth:google] WARNING: Token missing Drive scopes! Granted: ${tokens.scope}`);
        console.warn(`[oauth:google] This may cause limited file access (e.g., only 2 files visible)`);
      }
      ...
      if (existingAccount) {
        // Warn if existing account has limited scopes
        const existingScopes = (existingAccount.scopesJson as string[] | null) || [];
        const hasExistingDrive = existingScopes.some(s => s.includes("drive"));
        if (!hasExistingDrive) {
          console.warn(`[oauth:google] WARNING: Existing account lacks Drive scopes: ${existingScopes.join(", ")}`);
        }
  ...
  app.get("/api/oauth/slack", authMiddleware, (req, res) => {
    ...
    // Slack REQUIRES HTTPS. Priority: SLACK_PUBLIC_BASE_URL > REPLIT_DEV_DOMAIN > PUBLIC_BASE_URL
    ...
    // Diagnostic log for OAuth debugging
    console.log("[oauth:slack] client_id=%s redirect_uri=%s", clientId, redirectUri);
    console.log("[oauth:slack] env: SLACK_PUBLIC_BASE_URL=%s REPLIT_DEV_DOMAIN=%s",
      process.env.SLACK_PUBLIC_BASE_URL || "unset",
      process.env.REPLIT_DEV_DOMAIN || "unset");
...
Generate a playbook in JSON format matching the playbookResponseSchema.`;
  ...
  // Jobs Status API - Enhanced with progress tracking
  app.get("/api/jobs/scope/:scopeId/latest", authMiddleware, async (req: Request, res: Response) => {
    try {
      const { scopeId } = req.params;
      const job = await storage.getLatestJobByScope(scopeId);
      ...
      // Derive progress from stats_json or defaults
      const statsJson = latestRun?.statsJson as any || {};
      const progress = {
        phase: statsJson.phase || (job.status === "pending" ? "queued" : job.status === "completed" ? "done" : "unknown"),
        processedSources: statsJson.processedSources || 0,
        totalSources: statsJson.totalSources || null,
        processedChunks: statsJson.processedChunks || 0,
        totalChunksEstimate: statsJson.totalChunksEstimate || null,
        etaSeconds: statsJson.etaSeconds || null,
        startedAt: latestRun?.startedAt || null,
        updatedAt: statsJson.updatedAt || null,
        lastMessage: statsJson.lastMessage || null,
        error: latestRun?.error || statsJson.error || null
      };
      ...
      console.error("[jobs/scope/latest] Error:", e.message);
      res.status(500).json({ error: e.message });
    }
  });
