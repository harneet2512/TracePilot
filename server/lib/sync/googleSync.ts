...
async function fetchFolderContents(
  ...
  const fields = "nextPageToken,files(id,name,mimeType,modifiedTime,webViewLink,md5Checksum,parents)";
  ...
  for (const file of data.files) {
    if (file.mimeType === "application/vnd.google-apps.folder") {
      ...
      // Fetch parent folder metadata to get locationUrl
      let parentWebViewLink: string | undefined;
      if (file.parents && file.parents.length > 0) {
        const parentId = file.parents[0];
        try {
          const parentMeta = await fetchParentFolderMetadata(accessToken, parentId);
          parentWebViewLink = parentMeta?.webViewLink ||
                             `https://drive.google.com/drive/folders/${parentId}`;
        } catch (err) {
          console.warn(`[googleSync] Failed to fetch parent for ${file.name}:`, err);
          // Fallback: construct URL from parentId
          parentWebViewLink = `https://drive.google.com/drive/folders/${parentId}`;
        }
      }
      ...
      items.push({
        ...
        metadata: {
          ...
          parentWebViewLink,  // Store parent folder URL for locationUrl
...
async function fetchFileMetadata(accessToken: string, fileId: string): Promise<SyncableItem | null> {
  const fields = "id,name,mimeType,modifiedTime,webViewLink,md5Checksum,parents";
  ...
  // Fetch parent folder metadata for locationUrl
  let parentWebViewLink: string | undefined;
  if (file.parents && file.parents.length > 0) {
    const parentId = file.parents[0];
    try {
...
async function fetchParentFolderMetadata(
  accessToken: string,
  folderId: string
): Promise<{ webViewLink?: string } | null> {
  const url = `${GOOGLE_DRIVE_API}/files/${folderId}?fields=webViewLink&supportsAllDrives=true`;
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[googleSync] Failed to fetch parent folder ${folderId}: ${response.status}`);
      return null;
    }

    return response.json();
  } catch (error) {
    console.warn(`[googleSync] Error fetching parent folder ${folderId}:`, error);
    return null;
  }
}
...
async function fetchFileContent(
  ...
  if (mimeType === "application/pdf") {
    ...
    try {
      ...
      console.log(`[googleSync] PDF file ${fileId} downloaded (${buffer.byteLength} bytes) - extracting text...`);
