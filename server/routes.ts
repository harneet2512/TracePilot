...
import { classifyRegression, computeMetricDeltas, resolveBaselineRun, toLegacyDiffStatus, type BaselineMode } from "./lib/eval/regressionEngine";
...
export async function registerRoutes(
    ...
    try {
      ...
      if (conversationId) {
        ...
        if (!existing || existing.userId !== userId) {
          // If invalid or not owned, creating new to avoid FK crash or security issue
          console.log(`[Chat] Invalid conversationId ${conversationId}, creating new.`);
      ...
      const result = await runAgentTurn({
        ...
        workspaceId: req.user!.workspaceId || "default-workspace", // Pass workspace explicitly
      });
      ...
      const chatResponse: ChatResponse = {
        ...
        citations: result.citations, // Top-level citations from retrieval
        conversationId, // Return canonical ID
      };

      // 3. Persist Assistant Message
      // SANITIZATION: Do NOT store the full `chatResponse` recursively in metadata if it contains circular refs or huge data.
      const safeMetadata = {
        citations: result.citations,
        tokenUsage: { total: result.meta.tokensEstimate },
      ...
      const assistantMessage = await storage.createMessage({
        conversationId,
        role: "assistant",
        content: result.answerText, // Main text
      ...
      try {
        ...
        const { replyId } = await captureReplyArtifacts({
          ...
          retrieval: {
            ...
            dedupStats: { citationCount: citationArtifacts.length },
          },
        ...
        await enqueueJob({
          ...
          type: "score_reply",
          priority: 10,
          payload: { replyId, userPromptForJudge: message },
        });
      } catch (scoreErr) {
  ...
  // Debug RAG endpoint for retrieval diagnostics (dev only)
  app.get("/api/debug/rag", authMiddleware, async (req, res) => {
    try {
      ...
      const { q, workspaceId, scopeId } = req.query;
      ...
      const effectiveWorkspaceId = (workspaceId as string) || user.workspaceId || "default-workspace";
      const effectiveScopeId = scopeId as string | undefined;

      // Use retrieval pipeline with diagnostics
      const { retrieveForAnswer } = await import("./lib/retrieval");
      const result = await retrieveForAnswer(q, {
      ...
      res.json({
        query: q,
        workspaceIdUsed: result.diagnostics.workspaceIdUsed,
        scopeIdUsed: result.diagnostics.scopeIdUsed,
        chunksTotalInScope: result.diagnostics.existenceChecks.chunksTotalInScope,
        chunksWithKeywords: result.diagnostics.existenceChecks.chunksWithKeywords,
        primary: {
          count: result.diagnostics.primaryRetrieval.retrievedCount,
          topScore: result.diagnostics.primaryRetrieval.topScore,
          previews: result.diagnostics.primaryRetrieval.chunks,
        },
        fallback: result.diagnostics.fallbackLexical ? {
          count: result.diagnostics.fallbackLexical.retrievedCount,
          previews: result.diagnostics.fallbackLexical.chunks,
        } : null,
        final: {
          count: result.diagnostics.mergedReranked.retrievedCount,
          usedFallback: result.diagnostics.decision.usedFallback,
          reason: result.diagnostics.decision.reason,
          previews: result.diagnostics.mergedReranked.chunks,
        },
      });
    } catch (error) {
      console.error("Debug RAG error:", error);
      res.status(500).json({ error: "Debug RAG failed" });
    }
  });
...
{"answer": "...", "bullets": [], "action": null, "needsClarification": false, "clarifyingQuestions": []}`;
  ...
  app.get("/api/admin/chats/overview", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      const overview = await storage.getChatQualityOverview({
  ...
  app.get("/api/admin/chats/timeseries", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      const points = await storage.getChatQualityTimeseries({
        dateFrom: req.query.dateFrom ? new Date(String(req.query.dateFrom)) : undefined,
        dateTo: req.query.dateTo ? new Date(String(req.query.dateTo)) : undefined,
        environment: req.query.environment ? String(req.query.environment) : undefined,
        model: req.query.model ? String(req.query.model) : undefined,
  ...
  app.get("/api/admin/chats", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      ...
      const list = await storage.getAdminConversations({
        dateFrom: req.query.dateFrom ? new Date(String(req.query.dateFrom)) : undefined,
        dateTo: req.query.dateTo ? new Date(String(req.query.dateTo)) : undefined,
        environment: req.query.environment ? String(req.query.environment) : undefined,
        model: req.query.model ? String(req.query.model) : undefined,
      ...
      const rows = await Promise.all(list.rows.map(async (chat) => {
        ...
        const evalRows = await Promise.all(replies.map((reply) => storage.getEvalArtifact(reply.id)));
        const citationRows = await Promise.all(replies.map((reply) => storage.getCitationArtifact(reply.id)));
        const toolRows = await Promise.all(replies.map((reply) => storage.getToolArtifact(reply.id)));
        const latencies = replies.map((r) => r.latencyMs ?? 0).sort((a, b) => a - b);
        const p95LatencyMs = latencies.length ? latencies[Math.max(0, Math.ceil(latencies.length * 0.95) - 1)] : 0;
        const avgUnsupportedClaimRate = evalRows.length ? evalRows.reduce((s, row) => s + (row?.unsupportedClaimRate ?? 0), 0) / evalRows.length : 0;
        const citationIntegrityRate = citationRows.length ? citationRows.reduce((s, row) => s + (row?.citationIntegrityRate ?? 0), 0) / citationRows.length : 0;
        const flags: string[] = [];
        if (avgUnsupportedClaimRate > 0.2) flags.push("regression");
        if (avgUnsupportedClaimRate > 0.2 || citationIntegrityRate < 0.8) flags.push("needs_review");
        ...
        return {
          ...
          p95LatencyMs,
          totalTokens: replies.reduce((s, reply) => s + (reply.tokensIn ?? 0) + (reply.tokensOut ?? 0), 0),
          totalCostUsd: replies.reduce((s, reply) => s + (reply.costUsd ?? 0), 0),
          flags: [...new Set(flags)],
        };
      }));
      res.json({ page, pageSize, total: list.total, rows });
    } catch (error) {
  ...
  app.get("/api/admin/chats/:chatId/replies/:replyId", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      ...
      if (!reply || reply.chatId !== req.params.chatId) return res.status(404).json({ error: "Reply not found" });
      const messages = await storage.getMessages(chat.id);
  ...
  app.get("/api/admin/chats/baselines", authMiddleware, adminMiddleware, async (_req, res) => {
    try {
      ...
      res.json({
        ...
        runs: evalRuns.filter((run) => run.status === "completed").slice(0, 20),
      });
    } catch (error) {
