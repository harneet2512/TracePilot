...
const adminItems = [
  ...
  { title: "Chat Quality", url: "/admin/chats", icon: MessageSquare },
  { title: "Observability", url: "/admin/observability", icon: Activity },
];

// ChatList component for conversation management - uses API-based hooks
function ChatList({ currentPath }: { currentPath: string }) {
  const [, navigate] = useLocation();
  const [deleteConfirmId, setDeleteConfirmId] = useState<string | null>(null);

  // Use API-based hooks for unified state with ChatPage
      ...
      <SidebarHeader className="border-b border-sidebar-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-semibold text-sm truncate">TracePilot</span>
