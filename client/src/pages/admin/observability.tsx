...
interface ObservabilityChat {
  metrics: {
    ...
    tokenUsage: number;
    requestCount?: number;
    latencySplit?: { retrievalMs: number; generationMs: number; otherMs: number };
  };
...
export default function ObservabilityPage() {
  ...
  const { data: chatData, isLoading: chatLoading } = useQuery<ObservabilityChat>({
    ...
    queryFn: async () => {
      const range = buildRange(timeRange);
      const params = new URLSearchParams({ from: range.from, to: range.to, connector });
      const res = await fetch(`/api/admin/observability/chat?${params.toString()}`, { credentials: "include" });
  ...
  const { data: retrievalData, isLoading: retrievalLoading } = useQuery<ObservabilityRetrieval>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/admin/observability/retrieval?${params.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  });
  ...
  const { data: citationsData, isLoading: citationsLoading } = useQuery<ObservabilityCitations>({
    ...
    queryFn: async () => {
      const range = buildRange(timeRange);
      const params = new URLSearchParams({ from: range.from, to: range.to });
      const res = await fetch(`/api/admin/observability/citations?${params.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  });
  ...
  const { data: syncData, isLoading: syncLoading } = useQuery<ObservabilitySync>({
    queryKey: ["/api/admin/observability/sync", connector, timeRange],
    queryFn: async () => {
      const range = buildRange(timeRange);
      const params = new URLSearchParams({ from: range.from, to: range.to, connector });
      const res = await fetch(`/api/admin/observability/sync?${params.toString()}`, { credentials: "include" });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  });
  ...
  const noChatTelemetry = !chatData || (
    (chatData.metrics.totalConversations ?? 0) === 0 &&
    (chatData.metrics.tokenUsage ?? 0) === 0 &&
    (chatData.timeseries?.length ?? 0) === 0
  );
        ...
        <Tabs defaultValue="chat" className="space-y-4">
          ...
          <TabsContent value="chat" className="space-y-4">
            {noChatTelemetry && (
              <Card>
                <CardHeader>
                  <CardTitle>No chat telemetry yet</CardTitle>
                  <CardDescription>
                    Run chats and open reply detail pages to generate traces, spans, and evaluation artifacts.
                  </CardDescription>
                </CardHeader>
              </Card>
            )}
            ...
            <Card>
              <CardHeader>
                <CardTitle>Latency split and request volume</CardTitle>
                <CardDescription>Tied to observed spans in the selected time range.</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-4">
                <KpiCard title="Requests" value={chatData?.metrics.requestCount ?? 0} icon={Calendar} />
                <KpiCard title="Retrieval (avg ms)" value={`${chatData?.metrics.latencySplit?.retrievalMs ?? 0}ms`} icon={BarChart3} />
                <KpiCard title="Generation (avg ms)" value={`${chatData?.metrics.latencySplit?.generationMs ?? 0}ms`} icon={BarChart3} />
                <KpiCard title="Other (avg ms)" value={`${chatData?.metrics.latencySplit?.otherMs ?? 0}ms`} icon={BarChart3} />
              </CardContent>
            </Card>
