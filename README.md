...
### Local DB and Supabase DB Setup

TracePilot supports **SQLite** for local development (no Postgres required) and **PostgreSQL** for deployment (Supabase, Docker).

#### A) Local dev with SQLite (no DATABASE_URL)
...
#### B) Local dev with Docker Postgres
...
#### C) Supabase (Postgres) schema push

```powershell
# Windows PowerShell - set your Supabase connection string
...
**Scripts:**
- `npm run db:push` - Push schema (Postgres) or apply migrations (SQLite), auto-detects from DATABASE_URL
- `npm run db:push:pg` - Force Postgres push
- `npm run db:push:sqlite` - Force SQLite migrations
- `npm run db:doctor` - Validate connection and tables, suggest next steps
